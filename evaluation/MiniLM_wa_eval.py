"""Inference script for MiniLM baseline.
...
def main(
    ...
    model = MiniLM.from_pretrained(model_name, device=device, with_head=False, default_task="wa", load_wa_only=True).to(
        device
    )
    ...
    for filename in data_files:
        ...
        for example in tqdm(dataloader, total=len(dataloader), desc="Evaluating"):
            ...
            with torch.no_grad():
                prediction, logits = model.predict(inputs, task="wa", sampling=sampling)
            inf_end = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sampling", action="store_true", help="Set the sampling")
    parser.add_argument("--wandb", action="store_true", help="Set the wandb log")
    # Parse the arguments
