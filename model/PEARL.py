...
class PEARL(nn.Module):
    def __init__(
        ...
        if with_head:
            ...
            self.thr_head = initialize_classification_head(
                nn.Linear(hidden_size, max_thr - min_thr, dtype=torch.bfloat16, device=device)
            )
            self.bssid_head = initialize_classification_head(
                nn.Linear(hidden_size, 10, dtype=torch.bfloat16, device=device)
            )
            self.wa_head = initialize_classification_head(
                nn.Linear(hidden_size, len(index_to_parameter), dtype=torch.bfloat16, device=device)
            )
            self.wa_head_2layer = nn.Sequential(
                nn.Linear(hidden_size, 256, dtype=torch.bfloat16, device=device),
                nn.ReLU(),
                nn.Dropout(0.3),
                nn.Linear(256, len(index_to_parameter), dtype=torch.bfloat16, device=device),
            )
            self.wa_head_3layer = nn.Sequential(
                nn.Linear(hidden_size, 256, dtype=torch.bfloat16, device=device),
                nn.ReLU(),
                nn.Dropout(0.3),
                nn.Linear(256, 64, dtype=torch.bfloat16),
                nn.ReLU(),
                nn.Dropout(0.3),
                nn.Linear(64, len(index_to_parameter), dtype=torch.bfloat16, device=device),
            )
    ...
    @classmethod
    def from_pretrained(cls, path: str, device="cuda", **kwargs) -> "PEARL":
        ...
        if os.path.exists(path) and os.path.isdir(path):
            ...
            if config.get("merge_adapter", False):
                config["with_adapter"] = False
                base_model_name = path
            else:
                base_model_name = config.get("base_model_name", "unsloth/Llama-3.2-3B")
...
def initialize_classification_head(head):
    ...
    Args:
        head: Neural network layer to initialize

    Returns:
        Initialized head layer
    """
    return head
