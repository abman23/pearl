"""Supervised fine-tuning for PEARL model on Wi-Fi Aware parameter tuning."""
import argparse
import math
import os.path
...
def collate_fn(model):
    def _collate_fn(batch):
        ...
        if model.with_head:
            ...
            # Manually construct input_ids, attention_mask and labels for completion-only fine-tuning
            input_ids_list = []
            attention_mask_list = []
            labels_list = []
            max_length = -1

            # First pass: get max length, store input_ids and labels
            for example in batch:
                prompt = example["prompt"]
                opt_parameters = index_to_parameter[example["opt_parameters"]]
                label = f"({opt_parameters[0]}, {opt_parameters[1]})"
                prompt_tokens = model.tokenizer.encode(prompt, add_special_tokens=True)
                label_tokens = model.tokenizer.encode(f" {label}{model.tokenizer.eos_token}", add_special_tokens=False)
            ...
            # Second pass: pad sequences (left padding)
            for i in range(len(input_ids_list)):
                input_length = len(input_ids_list[i])
                padding_length = max_length - input_length
                input_ids_list[i] = [model.tokenizer.pad_token_id] * padding_length + input_ids_list[i]
                labels_list[i] = [-100] * padding_length + labels_list[i]

                # Create attention mask (1 for real tokens, 0 for padding)
                attention_mask = [0] * padding_length + [1] * input_length
                attention_mask_list.append(attention_mask)

            # Convert to tensors
            input_ids = torch.stack([torch.tensor(input_ids, dtype=torch.long) for input_ids in input_ids_list])
            attention_mask = torch.stack(
                [torch.tensor(attention_mask, dtype=torch.bool) for attention_mask in attention_mask_list]
            )
            labels = torch.stack([torch.tensor(labels, dtype=torch.long) for labels in labels_list])

        return (input_ids, attention_mask, labels, target_probs)

    return _collate_fn


def compute_loss(batch, model, device, criterion_ce=None, loss_type="kl"):
    input_ids, attention_mask, labels, target_probs = batch
...
def evaluate(model: PEARL, device, criterion_ce, max_eval_samples=-1, **kwargs):
...
def main(
    ...
    **kwargs,
    ...
    if with_head and with_adapter:
        project = "PEARL-WA-SFT-Head-Adapter"
    elif with_head and not with_adapter:
        project = "PEARL-WA-SFT-Head"
    elif not with_head and with_adapter:
        project = "PEARL-WA-SFT-Adapter"
    else:
        project = "PEARL-WA-SFT"
    wandb.init(
        project=project,
        name=new_model,
        config={
